...
public class GiveRandomCommand {
    ...
    private static int giveRandom(ServerCommandSource source, ServerPlayerEntity player, String typeName) {
        ...
        Item randomItem = type.randomSupplier.get();
    ...
    public enum Type {
        TM(() -> {
            ...
            return randomTm.getItem();
        }),
        TYPE_GEM(() -> {
            Optional<RegistryEntryList.Named<Item>> matches = Registries.ITEM.getEntryList(TagKey.of(Registries.ITEM.getKey(), Identifier.of("cobblemon", "type_gems")));
        ...
        private final Supplier<Item> randomSupplier;
        Type(Supplier<Item> randomSupplier) {
